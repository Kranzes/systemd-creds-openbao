...
The daemon is socket-activated: systemd owns the `AF_UNIX` socket and starts
the service on the first request, so what gets enabled is the socket unit,
not the service. Whatever the daemon authenticates as needs an OpenBao
policy covering the configured rules. [`-print-policy`](cli.md#-print-policy)
can generate one.
